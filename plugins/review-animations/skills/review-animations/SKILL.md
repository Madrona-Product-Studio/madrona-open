...
description: Review animation and motion code against a high craft bar. A senior motion-design reviewer that flags sluggish easing, comes-from-nowhere entrances, wrong transform origins, non-GPU properties, missing reduced-motion, and "it looks cool" animations on high-frequency actions — then returns a findings table plus a Block/Approve verdict. Use when reviewing a diff that touches CSS transitions, keyframes, springs, Framer Motion, or gesture code, or when asked whether an animation "feels right."
---

# Reviewing Animations

A specialized review skill. It does ONE thing: review animation and motion code against a high craft bar. It does not write features, fix unrelated bugs, or review non-motion code. If asked to review general code, decline and point to a general code-review skill.

The craft values below (frequency table, easing curves, durations, physicality, springs) draw on Emil Kowalski's animation philosophy ([animations.dev](https://animations.dev/)). The review *method* — non-negotiable standards, escalation triggers, a remedial hierarchy, tiered output, and explicit approval criteria — is original, adapted from aggressive code-quality review.
...
4. **Sub-300ms UI.** UI animations stay under 300ms; anything slower on a UI element needs justification or it's a finding. Per-element budgets are in the Reference below.
...
Be specific and cite `file:line`. When a value is needed (a curve, a duration, a spring config), pull the exact one from the Reference below rather than approximating.

## Seeing it, not just reading it

Code review catches wrong easing and non-GPU properties, but "feels right" is a visual judgment. When feel is uncertain, look at the animation instead of guessing:

- Bump the duration 2–5× (or use the DevTools Animations panel) and watch in slow motion — check colors crossfade cleanly, easing doesn't stop abruptly, `transform-origin` is right, coordinated properties stay in sync.
- Render the page/component and inspect it at desktop and mobile (~390px) widths. If the **screenshot** skill in this collection is installed, use it: `node shot.mjs <url> <outPath> [width] [height] [selector]`.
- Test gestures (drawers, swipe) on a real device — connect a phone, hit the dev server by IP.
- Look again with fresh eyes the next day; imperfections invisible during development surface later.
...
# Reference — the values behind the review

Cite these in findings instead of approximating. The craft values below are distilled from Emil Kowalski's design engineering philosophy ([animations.dev](https://animations.dev/)).
...
**Never animate keyboard-initiated actions** — they repeat hundreds of times daily; animation makes them feel slow and disconnected. Valid purposes for motion: spatial consistency, state indication, explanation, feedback, preventing jarring change. "It looks cool" on a frequently-seen element is not valid.
...
Feel natural because they simulate physics; no fixed duration — they settle on parameters. Use for: drag with momentum, "alive" elements, interruptible gestures, decorative mouse-tracking.
...
Keep bounce subtle (0.1–0.3); avoid bounce in most UI — reserve for drag-to-dismiss and playful interactions. Springs maintain velocity when interrupted (keyframes restart from zero), so they're ideal for gestures users may reverse mid-motion. For decorative mouse interactions, interpolate with a spring rather than tying the value directly to mouse position.
...
- **`clip-path: inset(t r b l)`** is a powerful animation tool: each value eats in from that side. Uses: reveal-on-scroll (`inset(0 0 100% 0)` → `inset(0 0 0 0)`), hold-to-delete overlay, seamless tab color transitions, comparison sliders.
...
- **Multi-touch protection**: ignore extra touch points after the drag begins (`if (isDragging) return`).
